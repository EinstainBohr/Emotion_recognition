...
Pnet::Pnet(){
    ...
    string filename = "/home/einstein/图片/MTCNN-light/Pnet.txt";
...
Rnet::Rnet(){
    ...
    string filename = "/home/einstein/图片/MTCNN-light/Rnet.txt";
    readData(filename, dataNumber, pointTeam);
...
Onet::Onet(){
    ...
    string filename = "/home/einstein/图片/MTCNN-light/Onet.txt";
    readData(filename, dataNumber, pointTeam);
...
void mtcnn::findFace(Mat &image){
    ...
    cv::rectangle(imageMask, Point(((*its).y1+(*its).y2)/2-10, ((*its).x1+(*its).x2)/2-10),
                             Point(((*its).y1+(*its).y2)/2+10, ((*its).x1+(*its).x2)/2+10),cv::Scalar(10),10);
    WatershedSegmenter segmenter;
    // Set markers and process
    segmenter.setMarkers(imageMask);
    segmenter.process(image);

    addWeighted(image,0.9,segmenter.getWatersheds(),0.5,0,image);
